                    ...
                    if(respArray[0].equals("GET"))
                    {
                        try
                        {
                            ...
                            if(!respArray[1].isEmpty())
                            {
                                ...
                                File peer_file = new File(path + File.separator + respArray[1]);
                                byte[] buffer = new byte[(int)peer_file.length()];
                                BufferedInputStream fileIn = new BufferedInputStream(new FileInputStream(peer_file));
...
public class Nap {
    ...
    public static void main(String[] args) {
        try {
            ...
            port = 7777;
            // System.out.print("请输入本机的文件目录 >> ");
            // path = stdin.readLine();
            path = "F:/CVPR 2018/train_color";
            Global.path = path;
            ...
            for(int i = 0; i < files.length; i++) {
                if(files[i].isFile()) {
                    ...
                    file_hash = DigestUtils.md5Hex(f_stream);
                    f_stream.close();
                    file_size = String.valueOf(files[i].length());

                    out.print("ADD " + file_name + " " + file_hash + " " + file_size);
                    out.flush();
                    response = in.readLine();
            ...
            do
            {
                ...
                if(request.equals("list"))
                {
                    ...
                    {
                        list_total++;
                        System.out.println(String.format("[%2d] : %20s [文件大小: %10s]",
                                new Object[] { new Integer(list_total), respArray[0], respArray[1] }));
                        ...
                        if(!reqArray[1].isEmpty()) {
                            ...
                            while((!respArray[0].equals("OK")) && (!respArray[0].equals("ERROR"))) {
                                ...
                                BufferedOutputStream fileOut = new BufferedOutputStream(new FileOutputStream(path
                                        + File.separator + "recv" + File.separator + reqArray[1]));
